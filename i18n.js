...
(function () {
  ...
  var T = {
    ...
    "zh-Hans": {
      ...
      "hero.badge": "右键唤起 AI 编程助手 · 支持 macOS 26 Liquid Glass",
      "hero.h1": "让右键菜单，<br><span class=\"grad\">回到它该有的样子</span>",
      "hero.sub": "右键在当前文件夹一键唤起 Claude、Gemini、Codex 等 AI 编程助手；再加上「新建文件」「在终端中打开」「复制路径」——把高频操作原生放回 Finder 右键菜单。轻量、快速、随叫随到。",
      "menu.openAITool": "打开 AI 工具", "menu.grpDomestic": "国内",
      "feat.ai.h": "右键打开 AI 工具",
      "feat.ai.p": "右键在当前文件夹用默认终端唤起 AI 编程 CLI：Claude Code、Gemini、Codex、Aider，以及国内的通义千问、Kimi、DeepSeek、MiniMax——自动检测已安装的工具，可勾选 <code>--dangerously-skip-permissions</code> 等参数或自定义。",
      ...
      "sup.faq.ai.s": "如何用右键「打开 AI 工具」？",
      "sup.faq.ai.p": "先在系统里装好对应的 AI 命令行工具（如 Claude Code、Gemini CLI、通义千问 qwen 等）。RightKit 会自动检测已安装的 CLI，在「设置 → 打开 AI 工具」中勾选要用的工具、选择默认终端；需要时可勾选 --dangerously-skip-permissions 等参数或自定义。之后右键任意文件夹 → 「打开 AI 工具」即可在该目录用默认终端运行。直接运行命令目前在 Terminal 与 iTerm 上支持最佳，其它终端会退化为仅打开目录。",
      "sup.other.h": "其他",
    ...
    "zh-Hant": {
      ...
      "hero.badge": "右鍵喚起 AI 程式設計助手 · 支援 macOS 26 Liquid Glass",
      "hero.h1": "讓右鍵選單，<br><span class=\"grad\">回到它該有的樣子</span>",
      "hero.sub": "右鍵在目前資料夾一鍵喚起 Claude、Gemini、Codex 等 AI 程式設計助手；再加上「新增檔案」「在終端機中開啟」「複製路徑」——把高頻操作原生放回 Finder 右鍵選單。輕量、快速、隨叫隨到。",
      "menu.openAITool": "打開 AI 工具", "menu.grpDomestic": "中國大陸",
      "feat.ai.h": "右鍵打開 AI 工具",
      "feat.ai.p": "右鍵在目前資料夾用預設終端機喚起 AI 程式設計 CLI：Claude Code、Gemini、Codex、Aider，以及中國大陸的通義千問、Kimi、DeepSeek、MiniMax——自動偵測已安裝的工具，可勾選 <code>--dangerously-skip-permissions</code> 等參數或自訂。",
      ...
      "sup.faq.ai.s": "如何用右鍵「打開 AI 工具」？",
      "sup.faq.ai.p": "先在系統裡裝好對應的 AI 命令列工具（如 Claude Code、Gemini CLI、通義千問 qwen 等）。RightKit 會自動偵測已安裝的 CLI，在「設定 → 打開 AI 工具」中勾選要用的工具、選擇預設終端機；需要時可勾選 --dangerously-skip-permissions 等參數或自訂。之後右鍵任意資料夾 → 「打開 AI 工具」即可在該目錄用預設終端機執行。直接執行命令目前在 Terminal 與 iTerm 上支援最佳，其它終端機會退化為僅打開目錄。",
      "sup.other.h": "其他",
    ...
    "en": {
      ...
      "hero.badge": "Summon AI coding agents from the right-click menu · macOS 26 Liquid Glass",
      "hero.h1": "Your right-click menu,<br><span class=\"grad\">the way it should be</span>",
      "hero.sub": "Right-click any folder to launch an AI coding agent — Claude, Gemini, Codex and more — in your default terminal. Plus New File, Open in Terminal and Copy Path: the actions that should have been there all along, natively back in the Finder menu.",
      "menu.openAITool": "Open AI Tool", "menu.grpDomestic": "China",
      "feat.ai.h": "Open AI tools from the right-click menu",
      "feat.ai.p": "Right-click any folder to launch an AI coding CLI in your default terminal: Claude Code, Gemini, Codex, Aider — plus China’s Qwen, Kimi, DeepSeek and MiniMax. Auto-detects what’s installed; opt into flags like <code>--dangerously-skip-permissions</code> or set your own.",
      ...
      "sup.faq.ai.s": "How do I use “Open AI Tool” from the right-click menu?",
      "sup.faq.ai.p": "First install the AI CLI you want (e.g. Claude Code, Gemini CLI, Qwen Code). RightKit auto-detects installed CLIs — in Settings → AI Tools, enable the ones you want and pick your default terminal; optionally opt into flags like --dangerously-skip-permissions or add your own. Then right-click any folder → “Open AI Tool” to run it in that directory. Running the command directly works best in Terminal and iTerm; other terminals just open the folder.",
      "sup.other.h": "More",
